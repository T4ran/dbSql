...
--데이터는 복제하지 않고 특정 테이블의 컬럼 형식만 가져오기
--조건문에 거짓인 조건을 넣고 실행한다.
...
--FOREIGN KEY 추가
--1. 실습 - dept 테이블에 deptno컬럼으로 PRIMARY KEY 제약을 테이블 변경
--ddl을 통하여 생성
ALTER TABLE dept ADD CONSTRAINT pk_dept PRIMARY KEY (deptno); 

--2. 실습 - emp 테이블에 empno컬럼으로 PRIMARY KEY 제약을 테이블 변경
--ddl을 통하여 생성
ALTER TABLE emp ADD CONSTRAINT pk_emp PRIMARY KEY (empno);

--3. emp 테이블의 deptno컬럼으로 dept 테이블의 deptno컬럼을 참조하는
--FK 제약을 테이블 변경 ddl을 통해 생성
--emp --> dept (deptno)
...
--CHECK 제약 추가
ALTER TABLE emp_test DROP CONSTRAINT ch_emp_test;
ALTER TABLE emp_test ADD CONSTRAINT ch_emp_test CHECK (LENGTH(ename)>=3);

INSERT INTO emp_test VALUES(9999,'brown',10);
INSERT INTO emp_test VALUES(9998,'sr',10);
rollback;
